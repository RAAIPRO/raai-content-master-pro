...
#
# Выход: ../zips-v<VER>/content-master-pro-v<VER>.zip set -euo pipefail SRC_DIR="$(cd "$(dirname "$0")" && pwd)"
SLUG="content-master-pro" # Версия из SKILL.md или переопределённая аргументом
if [[ -n "${1:-}" ]]; then VER="$1"
...
FILES=( SKILL.md README.md config.yaml .env.example install.sh test/smoke-test.sh examples/quick-start.md examples/full-library.md docs/onboarding.md docs/anti-fail.md docs/roi.md proof/case-01-chaos-to-strategy.md proof/case-02-no-seo-to-organic.md proof/case-03-time-reduction.md proof/case-04-email-reactivation.md proof/case-05-funnel-conversion.md
) # Опциональные (включаются если существуют)
OPTIONAL=( docs/competitors-comparison.md proof/dogfooding-RAAI.md proof/A1_content_strategy_raai_2026-04-17.md
...
for sub in marketing demo competitors; do if [[ -d "$SRC_DIR/$sub" ]]; then cp -R "$SRC_DIR/$sub" "$STAGE_DIR/$sub" echo " +dir: $sub/" fi
...
echo ""
echo "Contents:"
unzip -l "$OUT_DIR/$ZIP_NAME" | tail -40 # Smoke-test на свежераспакованном ZIP
if [[ "${SMOKE_TEST:-1}" != "0" ]]; then echo "" echo "==> Running smoke-test on fresh ZIP…" TEST_DIR="$(mktemp -d)" unzip -q "$OUT_DIR/$ZIP_NAME" -d "$TEST_DIR" cd "$TEST_DIR/${SLUG}-v${VER}" if [[ -x test/smoke-test.sh ]]; then bash test/smoke-test.sh || { echo "SMOKE-TEST FAILED"; exit 1; } else echo " (smoke-test.sh not executable or not found — skipped)" fi rm -rf "$TEST_DIR"
fi
